...
public:
    /// @brief Spatial dimensions of the input stream of events
    struct Parameters {
        int width, height;
        enum class Tuning { FastObjects, SlowObjects } tuning;

        Parameters() = default;
        Parameters(int width, int height, const Tuning &tuning) : width(width), height(height), tuning(tuning) {}
    };

    SparseOpticalFlowAlgorithm(const Parameters &parameters);
