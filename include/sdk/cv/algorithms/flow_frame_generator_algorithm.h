...
#ifndef METAVISION_SDK_CV_FLOW_FRAME_GENERATOR_ALGORITHM_H
#define METAVISION_SDK_CV_FLOW_FRAME_GENERATOR_ALGORITHM_H
...
class FlowFrameGeneratorAlgorithm {
public:
    ...
    void add_flow_for_frame_update(InputIt first, InputIt last) {
        ...
        last = std::prev(last);
        for (; first != last; --last) {
            auto resfind = std::find(ids_.begin(), ids_.end(), last->id);
            if (resfind == ids_.end()) {
...
#endif // METAVISION_SDK_CV_FLOW_FRAME_GENERATOR_ALGORITHM_H
