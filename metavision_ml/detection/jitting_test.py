...
def testcase_torch_jit_reset(nn_filename, height=120, width=160):
    ...
    model = torch.jit.load(nn_filename)
    model.reset_all()
    model.eval()
...
def testcase_compare_ckpt_vs_jit(ckpt_filename, jit_directory, height=120, width=160, sequence_raw_filename=None):
    ...
    with torch.no_grad():
        for _ in range(10):
            ...
            assert (torch.abs(loc_ckpt - loc_jit) < 1e-6).all()

            assert prob_ckpt.shape == prob_jit.shape
            assert (torch.abs(prob_ckpt - prob_jit) < 1e-6).all()
    ...
    delta_t = lightning_model.hparams["delta_t"]
    assert jit_json["delta_t"] == delta_t
    preproc_name = lightning_model.hparams["preprocess"]
