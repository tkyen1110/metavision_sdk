...
def load_boxes(metadata, batch_start_time, duration, tensor, **kwargs):
    ...
    If no bounding box is present in a frame but you still want to the
    training to consider to be annotated, you must annotate it with
    ...
    h_ratio = float(width_dst) / width_orig
    w_ratio = float(height_dst) / height_orig
    # assert abs(h_ratio - w_ratio) < 1e-6, "Rescale should keep aspect ratio"
...
def filter_empty_tensor(
        ...
        last_time_to_filter=None):
    """
    after a tensor of shape (num_bin, channels, h,w ) has been built thanks to the events
    we use it to discard boxes with no non nul data inside of it
    this is way more efficient than performing this computation on events directly
    """
    idx_to_filter = []
    # transfer the boxes coordinates in the tensor index system
    ...
    for i, (ts, xmin, xmax, ymin, ymax) in enumerate(zip(ts_a, xmin_a, xmax_a,
        ...
        if last_time_to_filter is not None and ts > last_time_to_filter:
            continue
        tmax = int(ts / time_per_bin) + 1
        min_t = max(0, tmax - int(100000 / time_per_bin))
        ymin = max(0, ymin)
        xmin = max(0, xmin)
        if ymax <= ymin or xmax <= xmin or tmax <= min_t:  # skipping zero-area boxes
            idx_to_filter.append(i)
            continue
        # we extract the slice corresponding to the box
        box_slice = np.abs(tensor[min_t:tmax, :, ymin:ymax, xmin:xmax])
