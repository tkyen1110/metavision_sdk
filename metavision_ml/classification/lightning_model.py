...
class ClassificationModel(pl.LightningModule):
    ...
    def __init__(self, hparams: argparse.Namespace) -> None:
        ...
        elif model_fn == models.ConvRNNClassifier:
    ...
    def _select_valid_frames(self, xs, targets, frame_is_labeled):
        if xs.dim() == 3:
            ...
            batch_size, num_classes = xs.shape
            targets = torch.tensor(targets).to(xs.device).view(batch_size)
    ...
    def training_step(self, batch, batch_nb):
        ...
        if y_hat.dim() == 3:
            ...
            train_dict = {"preds": yprob.view(y.shape[0], self.num_classes),  # [B, K]
                          "labels": y.to(torch.int),  # [B]
                          "info": batch['video_infos'],
                          "filter": filter_cls}  # [B]
    ...
    def _inference_step(self, batch, batch_idx, mode="val"):
        ...
        if y_hat.dim() == 3:
            ...
            inf_dict = {"preds": yprob.view(y.shape[0], self.num_classes),  # [B, K]
                        "labels": y.to(torch.int),  # [B]
                        "info": batch['video_infos'],
                        "filter": filter_cls}  # [B]
