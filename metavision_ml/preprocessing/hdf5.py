...
def _write(dic):
    ...
    write_to_hdf5(dic["path"], dic['start_ts'], output_folder=dic['output_folder'], delta_t=dic['delta_t'],
                  preprocess=dic['preprocess'], max_incr_per_pixel=dic['max_incr_per_pixel'],
                  height=dic['height'], width=dic['width'], max_duration=dic["max_duration"],
                  store_as_uint8=dic['store_as_uint8'])


def write_to_hdf5(path, start_ts=0, output_folder=".", delta_t=50000,
                  preprocess="histo", max_incr_per_pixel=5, height=None, width=None, max_duration=None,
                  store_as_uint8=False):
    """
    ...
    Args:
        ...
        max_incr_per_pixel (float): maximum number of increments per pixel (expressed in initial resolution).
    ...
    total_tbins_delta_t = num_tbins * delta_t
    ...
    cd_proc = CDProcessor(height, width, num_tbins=num_tbins,
                          preprocessing=preprocess, downsampling_factor=int(math.log2(event_input_height//height)),
    ...
    for index, events in tqdm(enumerate(mv_iterator), position=1):
        ...
        if max_duration is not None and index * total_tbins_delta_t - n_file * max_duration >= max_duration:
            ...
            if h5w.index:
                ...
                n_file += 1
                h5w = HDF5Writer(hfilename if max_duration is None else hfilename.format(n_file), "data",
                                 array_dim[1:], dtype=preprocess_dtype, attrs=attrs, store_as_uint8=store_as_uint8)
                h5w.dataset_size_increment = dataset_size_increment

        array = cd_proc(events, delta_t=total_tbins_delta_t)
...
def generate_hdf5(paths, output_folder, preprocess, delta_t, height=None, width=None,
                  start_ts=0, max_duration=None, max_incr_per_pixel=5., n_processes=2, box_labels=[],
                  store_as_uint8=False):
    """
    ...
    args_list = [{"path": path, "start_ts": start_ts, "output_folder": output_folder, "delta_t": delta_t,
                  "preprocess": preprocess, "max_incr_per_pixel": max_incr_per_pixel, "height": height, "width": width,
                  "max_duration": max_duration, "store_as_uint8": store_as_uint8}
