...
from metavision_sdk_cv import SparseOpticalFlowAlgorithm, SparseOpticalFlowAlgorithmParameters,  \
    FlowFrameGeneratorAlgorithm, SpatioTemporalContrastAlgorithm
...
def parse_args():
    ...
    noise_filtering_group = parser.add_argument_group("Noise Filtering", "Arguments related to STC noise filtering.")
    ...
    output_flow_group = parser.add_argument_group("Output flow", "Arguments related to output optical flow.")
...
def main():
    ...
    mv_iterator = EventsIterator(input_path=args.input_path, delta_t=args.accumulation_time_us)
    ...
    event_frame_gen = OnDemandFrameGenerationAlgorithm(width, height, args.accumulation_time_us)

    # Sparse Optical Flow Algorithm
    flow_algo = SparseOpticalFlowAlgorithm(width, height,
                                           tuning=SparseOpticalFlowAlgorithmParameters.Tuning.FastObjects)
    flow_buffer = SparseOpticalFlowAlgorithm.get_empty_output_buffer()

    # Flow Frame Generator
    flow_frame_gen = FlowFrameGeneratorAlgorithm()
    ...
    with Window(title="Metavision Sparse Optical Flow", width=width, height=height, mode=BaseWindow.RenderMode.BGR) as window:
        ...
        for evs in mv_iterator:
            ...
            if args.output_dense_h5_filename:
                all_dense_flows_start_ts.append(processing_ts - args.accumulation_time_us)
    ...
    if args.output_dense_h5_filename:
        ...
        flow_h5["flow"].attrs["input_file_name"] = os.path.basename(args.input_path)
