...
bool Pipeline::parse_command_line(int argc, char *argv[]) {
    ...
    input_options.add_options()
        ...
        ("input-raw-file,i", bpo::value<std::string>(&input_raw_file_)->default_value(""), "Path to input RAW file. If not specified, the camera live stream is used.")
    ...
    gui_options.add_options()
        ("no-display",  bpo::bool_switch(&no_display_), "Disable the GUI when reading a RAW (no effect with a live camera where GUI is already disabled).")
    ...
    const bool is_offline = !input_raw_file_.empty();
    ...
    if (input_raw_file_ != "") {
        try {
            camera_ = std::make_unique<Metavision::Camera>(
                Metavision::Camera::from_file(input_raw_file_, !as_fast_as_possible_));
        ...
        try {
            ...
            Metavision::Roi::Rectangle roi_rect;
