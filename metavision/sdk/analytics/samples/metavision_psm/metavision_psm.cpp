        ...
        try {
            ...
            if (roi_line_half_width_ >= 0) {
                ...
                if (transpose_axis_) {
                    ...
                    cols_to_enable.resize(camera_->geometry().width(), true); // Use all colmuns
...
bool Pipeline::parse_command_line(int argc, char *argv[]) {
    ...
    base_options.add_options()
        ...
        ("input-raw-file,i", bpo::value<std::string>(&filename_), "Path to input RAW file. If not specified, the camera live stream is used.")
