    ...
    if (!filename_.empty()) {
        try {
            camera_ = std::make_unique<Metavision::Camera>(
                Metavision::Camera::from_file(filename_, !replay_as_fast_as_possible_));
...
bool Pipeline::parse_command_line(int argc, char *argv[]) {
    ...
    outcome_options.add_options()
        ("no-display",            boost_po::bool_switch(&no_display_), "Disable the GUI when reading a RAW (no effect with a live camera where GUI is already disabled).")
