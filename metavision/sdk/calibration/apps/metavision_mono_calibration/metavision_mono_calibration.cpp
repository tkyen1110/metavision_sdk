...
bool Pipeline::refine_calibration() {
    ...
    cv::Mat img, img_undist, img_bgr, img_undist_bgr, concat_bgr;
    int i_selected = 0;
    ...
    for (SizeType i = 0; i < selected_views_.size(); ++i) {
        ...
        const std::vector<cv::Point2f> inner_corners{
            pts_2d_[i].front(),                                    // Top left
            pts_2d_[i][pattern_3d_.n_cols_ - 1],                   // Top right
            pts_2d_[i].back(),                                     // Bottom right
            pts_2d_[i][pattern_3d_.n_pts_ - pattern_3d_.n_cols_]}; // Bottom left
        ...
        cv::remap(img, img_undist, mapx, mapy, cv::INTER_CUBIC, 0, 255); // Set a white background around the chessboard
        cv::GaussianBlur(img_undist, img_undist,
        ...
        if (refinement_mode_ == CalibrationRefinementMode::REFINE_AND_SHOW_IMAGES) {
            ...
            cv::cvtColor(img_undist, img_undist_bgr, cv::COLOR_GRAY2BGR);
        }
        ...
        cv::undistortPoints(pts_2d_[i], keypoints, K_, d_);
        cv::perspectiveTransform(keypoints, keypoints, H.inv());
        ...
        find_corners_close_to_the_edge(pts_2d_[i], keypoints, i, corners_close_to_edge, corners_close_to_edge_indices);

        const float drow         = cv::norm(keypoints[0] - keypoints[1]);
        const float dcol         = cv::norm(keypoints[0] - keypoints[pattern_3d_.n_cols_]);
        const float search_rad_y = std::round(0.5 * (0.9 * drow - 1));
        ...
        cv::cornerSubPix(img_undist, keypoints, cv::Size(search_rad_x, search_rad_y), cv::Size(1, 1), criteria);

        // Overwrite possibly wrong subpix refinement for corners too close from the edge
        if (!corners_close_to_edge.empty()) {
            cv::cornerSubPix(img_undist, corners_close_to_edge, cv::Size(11, 11), cv::Size(1, 1), criteria);
...
void Pipeline::find_corners_close_to_the_edge(const std::vector<cv::Point2f> &input_pts,
    ...
    const int br_idx = pattern_3d_.n_rows_ * pattern_3d_.n_cols_ - 1;
    const int bl_idx = (pattern_3d_.n_rows_ - 1) * pattern_3d_.n_cols_;
...
void Pipeline::export_calibration_results() {
    ...
    cv::Mat export_rvecs;
    cv::hconcat(rvecs_, export_rvecs);
    fs << "rvecs" << export_rvecs.t();
    cv::Mat export_tvecs;
    cv::hconcat(tvecs_, export_tvecs);
    fs << "tvecs" << export_tvecs.t();
    fs.release();
